...
# Step 4: Determine critical value or p-value (two-tailed)
critical_value <- qt(1 - alpha/2, df = n-1)
p_value <- 2 * pt(abs(t_stat), df = n-1, lower.tail = FALSE) # Two-tailed p-value
...
abline(v = -critical_value, col = "red", lty = 2)  # Negative critical value for two-tailed test
